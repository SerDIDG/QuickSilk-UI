...
cm.getConstructor('App.ModuleImageGallery', function(classConstructor, className, classProto, classInherit){
    ...
    classProto.renderViewModel = function(){
        ...
        new cm.Finder('Com.GalleryPopup', that.params['name'], that.params['node'], function(classObject){
            that.components['gallery-popup'] = classObject;
            that.components['gallery-popup'].addEvent('onRequest', that.processPopupRequestHandler);
        });
    };
    ...
    classProto.processPage = function(pagination, page){
        ...
        // Add to gallery
        if(that.params['popup']){
            that.components['gallery-popup'] && that.components['gallery-popup'].collect(page['container']);
        ...
        // Finalize
        if(!itemsLength){
            that.components['pagination'].callbacks.finalize(that.components['pagination']);
        }
    };
    ...
    classProto.processItem = function(item){
        ...
        // Image
        item['descr'] = cm.node('div', {'class' : 'descr'});
